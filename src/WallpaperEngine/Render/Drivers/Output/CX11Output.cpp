...
void CustomXIOErrorExitHandler(Display *dsp, void *userdata)
{
    ...
    sLog.debugerror(
        "Critical XServer error detected. Attempting to recover...");
...
CX11Output::CX11Output(CApplicationContext &context, CVideoDriver &driver,
                       Detectors::CFullScreenDetector &detector)
    : COutput(context, detector)
    , m_driver(driver)
{
    // do not use previous handler, it might stop the app under weird
    // circumstances
...
void CX11Output::loadScreenInfo()
{
    ...
    {
        sLog.error("XRandr is not present, cannot detect specified screens, "
    ...
    this->m_fullWidth =
        DisplayWidth(this->m_display, DefaultScreen(this->m_display));
    this->m_fullHeight =
        DisplayHeight(this->m_display, DefaultScreen(this->m_display));
    XRRScreenResources *screenResources = XRRGetScreenResources(
        this->m_display, DefaultRootWindow(this->m_display));

    if (screenResources == nullptr)
    {
        sLog.error(
            "Cannot detect screen sizes using xrandr, running in window mode");
        return;
    }

    for (int i = 0; i < screenResources->noutput; i++)
    {
        XRROutputInfo *info = XRRGetOutputInfo(this->m_display, screenResources,
        ...
        XRRCrtcInfo *crtc =
            XRRGetCrtcInfo(this->m_display, screenResources, info->crtc);

        // add the screen to the list of screens
        this->m_screens.push_back(
            { { crtc->x, crtc->y, crtc->width, crtc->height }, info->name });
        ...
        {
            sLog.out("Found requested screen: ", info->name, " -> ", crtc->x,
                     "x", crtc->y, ":", crtc->width, "x", crtc->height);
    ...
    this->m_pixmap = XCreatePixmap(this->m_display, this->m_root,
                                   this->m_fullWidth, this->m_fullHeight, 24);
    this->m_gc = XCreateGC(this->m_display, this->m_pixmap, 0, nullptr);
    // pre-fill it with black
    XFillRectangle(this->m_display, this->m_pixmap, this->m_gc, 0, 0,
    ...
    this->m_image = XCreateImage(this->m_display, CopyFromParent, 24, ZPixmap,
                                 0, this->m_imageData, this->m_fullWidth,
...
void CX11Output::updateRender() const
{
    ...
    XPutImage(this->m_display, this->m_pixmap, this->m_gc, this->m_image, 0, 0,
              0, 0, this->m_fullWidth, this->m_fullHeight);
    ...
    // it also forces the compositor to refresh the background (tested with
    // picom)
    Atom prop_root = XInternAtom(this->m_display, "_XROOTPMAP_ID", False);
    Atom prop_esetroot =
        XInternAtom(this->m_display, "ESETROOT_PMAP_ID", False);
    XChangeProperty(this->m_display, this->m_root, prop_root, XA_PIXMAP, 32,
                    PropModeReplace, (unsigned char *)&this->m_pixmap, 1);
    XChangeProperty(this->m_display, this->m_root, prop_esetroot, XA_PIXMAP, 32,
                    PropModeReplace, (unsigned char *)&this->m_pixmap, 1);
